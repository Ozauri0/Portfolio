...
export default function AdminAccess() {
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <Card className="w-full max-w-md bg-zinc-900 border-zinc-800 mx-4">
      ...
      <div className="container mx-auto px-4 py-4 md:py-8">
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                ...
                <Database className="h-6 w-6 md:h-8 md:w-8 text-purple-400" />
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardContent className="p-4 md:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
                <h3 className="text-lg md:text-xl font-bold text-white">Clicks en Redes Sociales</h3>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardContent className="p-4 md:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
                <h3 className="text-lg md:text-xl font-bold text-white">Clicks en Proyectos</h3>
                <div className="flex items-center space-x-2">
          ...
          <CardContent className="p-4 md:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
              <div className="flex items-center space-x-3">
                ...
                <h3 className="text-lg md:text-xl font-bold text-white">Últimas Conexiones</h3>
              </div>
              <Button 
            ...
            {loadingRecentLogs ? (
              ...
              <div className="space-y-3">
                {recentLogs.map((log) => (
                  <div key={log.id} className="p-3 md:p-4 bg-zinc-800 rounded-lg border border-zinc-700">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 gap-2">
                      <div className="flex items-center space-x-2">
                        <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                        <span className="text-white font-medium truncate">{log.email}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-3 text-sm">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-gray-300 truncate">{log.formattedDate}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <MapPin className="h-4 w-4 text-purple-400 flex-shrink-0" />
                        <span className="text-gray-300 truncate">{log.ipAddress}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Monitor className="h-4 w-4 text-blue-400 flex-shrink-0" />
                        <span className="text-gray-300 truncate">{getBrowserInfo(log.userAgent)} - {getDeviceInfo(log.userAgent)}</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* All Logs Table - Mejorar responsividad */}
        {showAllLogs && (
          <Card className="bg-zinc-900 border-zinc-800 mb-6">
            <CardContent className="p-4 md:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
                <h3 className="text-lg md:text-xl font-bold text-white">Historial Completo de Conexiones</h3>
                <Button 
                  ...
                  <table className="min-w-full divide-y divide-gray-700">
                    <thead className="sticky top-0 bg-zinc-800">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden sm:table-cell">Fecha</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden md:table-cell">IP</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden lg:table-cell">Ubicación</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden md:table-cell">Navegador</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden lg:table-cell">Dispositivo</th>
                    ...
                    <tbody className="bg-zinc-800 divide-y divide-gray-700">
                      {allLogs.map((log) => (
                        <tr key={log.id} className="hover:bg-zinc-700">
                          <td className="px-4 py-3 text-sm text-white truncate max-w-[150px]">{log.email}</td>
                          <td className="px-4 py-3 text-sm text-white hidden sm:table-cell">{log.formattedDate}</td>
                          <td className="px-4 py-3 text-sm text-white font-mono hidden md:table-cell">{log.ipAddress}</td>
                          <td className="px-4 py-3 text-sm text-white hidden lg:table-cell">{log.location}</td>
                          <td className="px-4 py-3 text-sm text-white hidden md:table-cell">{getBrowserInfo(log.userAgent)}</td>
                          <td className="px-4 py-3 text-sm text-white hidden lg:table-cell">{getDeviceInfo(log.userAgent)}</td>
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardContent className="p-4 md:p-6">
            <h3 className="text-lg md:text-xl font-bold text-white mb-4">Información del Sistema</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="text-white">JWT + Refresh Tokens</p>
              </div>
            </div>
          </CardContent>
        </Card>
